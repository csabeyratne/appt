...
var defaults = {
	...
	unselectAuto: true,
	asyncAjax: true, 
	invisibleEvents: 'Available_sdsdfdfs',  // rhess -- config?
	dropAccept: '*'
...
function EventManager(options, eventSources) {
	...
	function _fetchEventSource(src, callback) {
		...
		if (typeof src == 'string') {
			...
			$.ajax({
				url: src,
				async: options.asyncAjax, // needed for a bug in fetching ! rhess TODO report the bug
				dataType: 'json',
				data: params,
		...
		if (!head) { // first time rendering, build from scratch
			...
			for (i=0; d < maxd; i++) {
				...
				s += "<tr class='" +
					...
					"'>";
					if (opt('showTimeSlot')) { 
						s+=	"<th class='fc-axis fc-leftmost " + tm + "-state-default'>" +
...
function AgendaEventRenderer() {
	...
	//log('renderSlotSegs '+segCnt);		
		...
		for (i=0; i<segCnt; i++) {
			...
			availWidth = Math.min(availWidth-6, availWidth*.90 ); // TODO: move this to CSS
	//log('levelI '+levelI);	
			if (levelI) {
				// indented and thin
...
function View(element, calendar, viewName) {
	...
	t.eachElementOnDay = eachElementOnDay;
	// t.title
	// t.start, t.end
	// t.visStart, t.visEnd
	...
	function reportEventElement(event, element) {
		...
		var key = event.start.toDateString();	// track events by date for month view
		if (eventElementsByDate[key]) {
			eventElementsByDate[key].push(element);
		}else{
			eventElementsByDate[key] = [element];
		}
	}
	
	function eachElementOnDay(event) {
		var key = event.start.toDateString();   
		var ret = eventElementsByDate[key];
		
		return eventElementsByDate[key];
	}
...
function segsCollide(seg1, seg2) {
	// rhess -- if (!seg1.event.viz || !seg2.event.viz) return false; // invisible events take no space
		...
		while (true) {
			...
			if (levels[j]) {
				for (k=0; k<levels[j].length; k++) {
					...
					// rhess -- testing this 
					if ( levels[j][k].event.type == defaults.invisibleEvents ) continue;
